...
class ReplaceScoreState : public State
{
public:
	ReplaceScoreState(int Score) : Selected(0), Up(false), Down(false), Enter(false), Score(Score), A(false)
	{
		Type = "Replace Score";
	...
	void WriteScore()
	{
		ScoreList[Selected] = Score;

		std::ofstream Ofs("Scores/scoreslist.txt", std::ios_base::out);
		for (auto i = 0U; i < ScoreList.size(); ++i)
			Ofs << ScoreList[i] << "\n";
	}
	...
	void Update(sf::RenderWindow & Window, float ElapsedTime) override
	{
		...
		if (!sf::Joystick::isButtonPressed(0, 0))
			if (Selected >= 0 && Selected < static_cast<int>(ScoreList.size()) && A)
			{
				WriteScore();
				while (StateStack.size() > 1)
					StateStack.pop_back();
			}
			else if (Selected == ScoreList.size() && A)
				StateStack.pop_back();

		if (!sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Return))
			if (Selected >= 0 && Selected < static_cast<int>(ScoreList.size()) && Enter)
			{
				WriteScore();
				while (StateStack.size() > 1)
					StateStack.pop_back();
			}
			else if (Selected == ScoreList.size() && Enter)
				StateStack.pop_back();
